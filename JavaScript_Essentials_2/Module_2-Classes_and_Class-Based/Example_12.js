...
// Create a User class to create objects for both teachers and students. The constructor should take the user data (name, surname, email, role), 
// but be sure to create the appropriate properties.

// Additionally, create the following methods:

// - addCourse(course, level) - which will allow you to add course (e.g. math) and level (e.g. 2 - the higher the number, the higher the level); in the case of a student, 
// it will mean that they are looking for help on this level, and in case of a teacher, it will mean that they can help up to this level;
...
// - sendMessage(from, message) - which will allow you to send a 'message' message from user 'from' to the user described in the object; complete information about the sent 
// message should be stored in the local cache (hint: use an array for this); the sending of the message itself will only be simulated, declare the function 
// sendEmail(from, to, message) {} beforehand and use it in the appropriate place;
...
teacher1.showMessagesHistory(); // -> rfife@rhyta.com -> PaulaThompkins@jourrapide.com: test message
